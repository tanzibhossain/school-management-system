...
| 22 | LMS *(optional)* | Academic, Student | 🔶 code complete 2026-07-04 — `app/Modules/LMS` (Course, Lesson, Assignment, Submission, SubmissionAiCheck); real Anthropic API integration (not stubbed — see decisions below); gated by the new `school_module_settings` toggle (retrofitted onto Payroll too); awaiting test run in Docker |
