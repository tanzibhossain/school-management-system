...
| 20 | Website | — | ⬜ pending — **design locked 2026-07-04, not yet built** (full plan in SESSION_START.md's "Module 20: Website" section — read it before starting, it captures decisions already made so the research/clarification rounds don't need repeating): full DevPlan Sprint-1 backend scope (9 tables: menus, menu_items, site_settings, pages, page_redirects, page_layouts, site_layouts, page_templates, website_media); layout stored as an opaque `layout_json` blob (Next.js/Craft.js owns block structure, Laravel never parses it); dynamic-block public reads included in this pass (Result Checker needs a NEW public query in the Website module itself — Mark has no public result lookup and must NOT be modified; Notice Board reuses `AnnouncementRepository::listVisible()` as-is; Staff/Teacher List filters by `designation_id`/`department_id` since Staff has no subject relation; Class Routine joins `class_routines`; Admission Form needs no new code, it's OnlineAdmission's existing public endpoint) |
