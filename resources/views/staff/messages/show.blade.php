...
@section('content')
  ...
  <div class="mb-3">
    <a href="{{ route('staff.messages') }}" class="text-decoration-none small"><i class="bi bi-arrow-left me-1"></i>{{ __('Back to messages') }}</a>
  ...
  @else
    <form method="POST" action="{{ route('staff.messages.reply', $thread->id) }}">
      @csrf
