...
@section('content')
  ...
  <div class="mb-3"><a href="{{ route('admin.transport.routes.index') }}" class="text-decoration-none small"><i class="bi bi-arrow-left"></i> Back to routes</a></div>

  <div class="row g-4">
    <div class="col-lg-5">
      <div class="card mb-4"><div class="card-header">Vehicle</div><div class="card-body">
        ...
        <form method="POST" action="{{ route('admin.transport.routes.set-vehicle', $route->id) }}" class="row g-2">
          ...
          <div class="col-4"><button class="btn btn-sm btn-primary w-100">Set</button></div>
        </form>
      </div></div>

      <div class="card"><div class="card-header">Details</div><div class="card-body">
        <dl class="row mb-0">
          <dt class="col-5 text-muted">Fare</dt><dd class="col-7">{{ number_format((float) $route->fare, 2) }}</dd>
          <dt class="col-5 text-muted">Driver</dt><dd class="col-7">{{ $route->driver?->name ?? '—' }}</dd>
          @if ($route->description)<dt class="col-5 text-muted">Description</dt><dd class="col-7">{{ $route->description }}</dd>@endif
        </dl>
      </div></div>
    </div>
    ...
    <div class="col-lg-7">
      <div class="card"><div class="card-header d-flex justify-content-between align-items-center">
        ...
        <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#assignModal" {{ $route->current_vehicle_id ? '' : 'disabled' }}><i class="bi bi-plus-lg"></i> Add rider</button>
      </div><div class="card-body">
        @unless ($route->current_vehicle_id)
          <div class="alert alert-warning py-2 small">Assign an operational vehicle before adding riders.</div>
        @endunless
        @if ($riders->isEmpty())
          <p class="text-muted mb-0">No active riders.</p>
        @else
          <table class="table align-middle mb-0">
            <thead><tr><th>Student</th><th>Pickup</th><th>Since</th><th class="text-end" data-orderable="false"></th></tr></thead>
            <tbody>
              @foreach ($riders as $a)
                <tr>
  ...
  <div class="modal fade" id="assignModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form method="POST" action="{{ route('admin.transport.routes.riders.assign', $route->id) }}">
      @csrf
      <div class="modal-header"><h5 class="modal-title">Add rider</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
      <div class="modal-body row g-3">
        ...
        <div class="col-md-7"><label class="form-label">Pickup point</label>
          <input name="pickup_point" class="form-control"></div>
        <div class="col-md-5"><label class="form-label">Starts on</label>
