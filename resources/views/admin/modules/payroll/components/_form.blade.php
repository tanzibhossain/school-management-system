  ...
  <form method="POST" action="{{ $action }}">
    ...
    <div class="modal-body row g-3">
      <div class="col-12"><label class="form-label">Name <span class="text-danger">*</span></label>
        <input name="name" class="form-control" value="{{ $isEdit ? $c->name : old('name') }}" placeholder="e.g. Basic, House rent, PF" required></div>
      <div class="col-md-6"><label class="form-label">Type <span class="text-danger">*</span></label>
        <select name="component_type" class="form-select" required>
          <option value="earning" @selected(($isEdit ? $c->component_type : old('component_type'))==='earning')>Earning</option>
          <option value="deduction" @selected(($isEdit ? $c->component_type : old('component_type'))==='deduction')>Deduction</option>
        </select></div>
      ...
      <div class="col-12"><div class="form-check"><input type="hidden" name="is_default" value="0"><input class="form-check-input" type="checkbox" name="is_default" value="1" id="def{{ $modalId }}" @checked($isEdit ? $c->is_default : false)><label class="form-check-label" for="def{{ $modalId }}">Default (applies to new staff)</label></div></div>
    </div>
