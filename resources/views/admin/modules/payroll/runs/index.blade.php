...
@section('content')
  ...
  <div class="card"><div class="card-body">
    <table class="table table-hover align-middle w-100 js-dt">
      <thead><tr><th>{{ __('Period') }}</th><th>{{ __('Status') }}</th><th>{{ __('Entries') }}</th><th class="text-end" data-orderable="false">{{ __('Actions') }}</th></tr></thead>
      <tbody>
        @foreach ($runs as $r)
          <tr>
  ...
  <div class="modal fade" id="createModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form method="POST" action="{{ route('admin.payroll.runs.store') }}">
      ...
      <div class="modal-body row g-3">
        ...
        <div class="col-md-6"><label class="form-label">{{ __('Year') }} <span class="text-danger">*</span></label>
          <input type="number" min="2000" max="2100" name="year" class="form-control" value="{{ now()->year }}" required></div>
        <div class="col-12"><label class="form-label">{{ __('Notes') }}</label><input name="notes" class="form-control"></div>
      </div>
