  ...
  <div class="d-flex justify-content-between align-items-start mb-3 flex-wrap gap-2">
    ...
    <div class="d-flex gap-2">
      @if ($run->status === 'draft')
        ...
        @if ($hasEntries)
          <form method="POST" action="{{ route('admin.payroll.runs.approve', $run->id) }}" onsubmit="return confirm('Approve this run? It becomes final.')">@csrf @method('PATCH')<button class="btn btn-success"><i class="bi bi-check2-all"></i> {{ __('Approve') }}</button></form>
        @endif
      @endif
    </div>
  </div>
  ...
  <div class="card"><div class="card-body">
    ...
    @else
      <table class="table table-hover align-middle w-100 js-dt">
        <thead><tr><th>{{ __('Employee') }}</th><th>{{ __('Name') }}</th><th class="text-end">{{ __('Gross') }}</th><th class="text-end">{{ __('Deductions') }}</th><th class="text-end">{{ __('Net') }}</th></tr></thead>
        <tbody>
