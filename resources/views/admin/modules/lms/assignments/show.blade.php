...
@section('content')
  ...
  <div class="mb-3"><a href="{{ route('admin.lms.courses.show', $assignment->course_id) }}" class="text-decoration-none small"><i class="bi bi-arrow-left"></i> Back to course</a></div>
  ...
  <div class="card"><div class="card-body">
    ...
    @else
      <table class="table align-middle mb-0">
        <thead><tr><th>Student</th><th>Submitted</th><th>Late</th><th>AI check</th><th class="text-end">Marks</th><th class="text-end" data-orderable="false">Grade</th></tr></thead>
        <tbody>
          @foreach ($submissions as $s)
            <tr>
              <td class="fw-semibold">{{ $s->student?->name ?? '—' }}</td>
              <td class="small">{{ optional($s->submitted_at)->format('d M Y H:i') }}</td>
              <td>@if ($s->late_submission)<span class="badge text-bg-warning">Late</span>@else — @endif</td>
              <td>
                @if ($s->aiCheck)
              ...
              <td class="text-end">
                <form method="POST" action="{{ route('admin.lms.submissions.grade', $s->id) }}" class="d-inline-flex gap-1">
                  ...
                  <input name="feedback" class="form-control form-control-sm" style="width:10rem" placeholder="Feedback" value="{{ $s->teacher_feedback }}">
