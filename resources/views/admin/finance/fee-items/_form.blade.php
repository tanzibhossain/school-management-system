...
<div class="modal fade" id="{{ $modalId }}" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content">
  <form method="POST" action="{{ $action }}">
    ...
    <div class="modal-body row g-3">
      ...
      <div class="col-md-3"><label class="form-label">Amount <span class="text-danger">*</span></label>
        <input type="number" step="0.01" min="0" name="amount" class="form-control" value="{{ $isEdit ? $it->amount : old('amount') }}" required></div>
      <div class="col-md-3"><label class="form-label">Frequency <span class="text-danger">*</span></label>
      ...
      <div class="col-md-4"><label class="form-label">Class <span class="text-muted small">(blank = all)</span></label>
      ...
      <div class="col-md-3"><label class="form-label">Due day <span class="text-muted small">(1–28)</span></label>
        <input type="number" min="1" max="28" name="due_day" class="form-control" value="{{ $isEdit ? $it->due_day : old('due_day') }}"></div>
      <div class="col-md-9 d-flex align-items-end">
        <div class="form-check"><input type="hidden" name="is_mandatory" value="0"><input class="form-check-input" type="checkbox" name="is_mandatory" value="1" id="mand{{ $isEdit ? $it->id : 'New' }}" @checked($isEdit ? $it->is_mandatory : true)><label class="form-check-label" for="mand{{ $isEdit ? $it->id : 'New' }}">Mandatory</label></div>
      </div>
    </div>
