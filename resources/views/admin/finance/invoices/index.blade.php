...
@section('content')
  <div class="d-flex justify-content-between align-items-start mb-3 flex-wrap gap-2">
    <div>
      <nav><ol class="breadcrumb small mb-1"><li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}" class="text-decoration-none">Home</a></li><li class="breadcrumb-item">Finance</li><li class="breadcrumb-item active">Invoices</li></ol></nav>
      <h1 class="h4 mb-0">Invoices</h1>
    </div>
    <div class="d-flex gap-2">
      <button class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#bulkModal"><i class="bi bi-collection"></i> Bulk generate</button>
      <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#singleModal"><i class="bi bi-plus-lg"></i> Generate invoice</button>
    </div>
  </div>
  ...
  <form method="GET" class="card mb-3"><div class="card-body row g-2 align-items-end">
    ...
    <div class="col-sm-4"><button class="btn btn-sm btn-outline-primary">Filter</button>
      <a href="{{ route('admin.invoices.index') }}" class="btn btn-sm btn-outline-secondary">Reset</a></div>
  </div></form>

  <div class="card"><div class="card-body">
    <table class="table table-hover align-middle w-100 js-dt">
      <thead><tr><th>Invoice #</th><th>Student</th><th>Month</th><th>Due</th><th>Paid</th><th>Status</th><th class="text-end" data-orderable="false">Actions</th></tr></thead>
      <tbody>
  ...
  <div class="modal fade" id="singleModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form method="POST" action="{{ route('admin.invoices.generate-single') }}">
      ...
      <div class="modal-body row g-3">
        ...
        <div class="col-md-6"><label class="form-label">Month <span class="text-muted small">(optional)</span></label>
        ...
        <div class="col-md-6"><label class="form-label">Discount <span class="text-muted small">(optional)</span></label>
  ...
  <div class="modal fade" id="bulkModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form method="POST" action="{{ route('admin.invoices.generate-bulk') }}">
      @csrf
      <div class="modal-header"><h5 class="modal-title">Bulk generate by class</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
      <div class="modal-body row g-3">
        ...
        <div class="col-12"><label class="form-label">Due date <span class="text-danger">*</span></label>
          <input type="date" name="due_date" class="form-control" required></div>
