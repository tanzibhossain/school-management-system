    ...
    <div class="modal-body row g-3">
      ...
      <div class="col-md-5"><label class="form-label">Type <span class="text-danger">*</span></label>
        <select name="type" class="form-select" required>
          <option value="percentage" @selected(($isEdit ? $d->type : old('type'))==='percentage')>Percentage</option>
          <option value="fixed" @selected(($isEdit ? $d->type : old('type'))==='fixed')>Fixed amount</option>
        </select></div>
      <div class="col-md-3"><label class="form-label">Value <span class="text-danger">*</span></label>
        <input type="number" step="0.01" min="0" name="value" class="form-control" value="{{ $isEdit ? $d->value : old('value') }}" required></div>
      <div class="col-md-4"><label class="form-label">Max amount</label>
