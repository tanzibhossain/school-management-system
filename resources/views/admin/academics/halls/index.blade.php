...
@section('content')
  ...
  <div class="card"><div class="card-body">
    <table class="table table-hover align-middle w-100 js-dt">
      <thead><tr><th>Name</th><th>Description</th><th>Seats</th><th>Available</th><th class="text-end" data-orderable="false">Actions</th></tr></thead>
      <tbody>
        @foreach ($halls as $h)
          <tr>
            ...
            <td class="text-end">
              <a class="btn btn-sm btn-outline-primary" href="{{ route('admin.exam-halls.show', $h->id) }}">Seats</a>
  ...
  <div class="modal fade" id="createModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form method="POST" action="{{ route('admin.exam-halls.store') }}">
      @csrf
      <div class="modal-header"><h5 class="modal-title">New exam hall</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
      <div class="modal-body row g-3">
        <div class="col-12"><label class="form-label">Name <span class="text-danger">*</span></label>
          <input name="name" class="form-control" value="{{ old('name') }}" placeholder="e.g. Main Hall" required></div>
        <div class="col-12"><label class="form-label">Description</label>
          <input name="description" class="form-control" value="{{ old('description') }}"></div>
        <div class="col-md-4"><label class="form-label">Rows <span class="text-danger">*</span></label>
          <input type="number" min="1" max="100" name="rows" class="form-control" value="{{ old('rows', 10) }}" required></div>
        <div class="col-md-4"><label class="form-label">Left seats/row <span class="text-danger">*</span></label>
          <input type="number" min="1" max="20" name="left_per_row" class="form-control" value="{{ old('left_per_row', 3) }}" required></div>
        <div class="col-md-4"><label class="form-label">Right seats/row</label>
          <input type="number" min="0" max="20" name="right_per_row" class="form-control" value="{{ old('right_per_row', 3) }}"></div>
        <div class="col-12"><div class="alert alert-info py-2 mb-0 small">Seats are generated as <code>R01-L1, R01-L2, … R01-R1 …</code> per row. You can block individual seats afterwards.</div></div>
      </div>
