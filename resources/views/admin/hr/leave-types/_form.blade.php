  ...
  <form method="POST" action="{{ $action }}">
    ...
    <div class="modal-body row g-3">
      <div class="col-md-7"><label class="form-label">{{ __('Name') }} <span class="text-danger">*</span></label>
        <input name="name" class="form-control" value="{{ $isEdit ? $t->name : old('name') }}" placeholder="{{ __('E.g. Sick, Casual') }}" required></div>
      <div class="col-md-5"><label class="form-label">{{ __('Applies To') }} <span class="text-danger">*</span></label>
      ...
      <div class="col-md-5"><label class="form-label">{{ __('Max Days / Year') }}</label>
      ...
      <div class="col-md-7 d-flex align-items-end gap-4">
        <div class="form-check"><input type="hidden" name="requires_attachment" value="0"><input class="form-check-input" type="checkbox" name="requires_attachment" value="1" id="att{{ $modalId }}" @checked($isEdit ? $t->requires_attachment : false)><label class="form-check-label" for="att{{ $modalId }}">{{ __('Attachment') }}</label></div>
        <div class="form-check"><input type="hidden" name="is_paid" value="0"><input class="form-check-input" type="checkbox" name="is_paid" value="1" id="paid{{ $modalId }}" @checked($isEdit ? $t->is_paid : false)><label class="form-check-label" for="paid{{ $modalId }}">{{ __('Paid') }}</label></div>
      </div>
    </div>
