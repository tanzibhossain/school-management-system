...
@section('content')
  @include('admin.partials.page-header', ['title' => __('Student leave requests'), 'crumbs' => [__('HR'), __('Student leave')]])
  ...
  <form method="GET" class="card mb-3"><div class="card-body row g-2 align-items-end">
    ...
    <div class="col-sm-8"><button class="btn btn-sm btn-outline-primary">{{ __('Filter') }}</button>
      <a href="{{ route('admin.student-leave.index') }}" class="btn btn-sm btn-outline-secondary">{{ __('Reset') }}</a></div>
  </div></form>

  @php $m = ['pending'=>'warning','approved'=>'success','rejected'=>'danger','cancelled'=>'secondary']; @endphp
  <div class="card"><div class="card-body">
    <table class="table table-hover align-middle w-100 js-dt">
      <thead><tr><th>{{ __('Student') }}</th><th>{{ __('Type') }}</th><th>{{ __('From') }}</th><th>{{ __('To') }}</th><th>{{ __('Days') }}</th><th>{{ __('Status') }}</th><th class="text-end" data-orderable="false">{{ __('Actions') }}</th></tr></thead>
      <tbody>
        @foreach ($requests as $r)
          <tr>
            ...
            <td class="text-end">
              @if ($r->status === 'pending')
                <form method="POST" action="{{ route('admin.student-leave.approve', $r->id) }}" class="d-inline">@csrf @method('PATCH')<button class="btn btn-sm btn-outline-success">{{ __('Approve') }}</button></form>
                <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#rejectModal{{ $r->id }}">{{ __('Reject') }}</button>
              @elseif ($r->status === 'approved')
                <form method="POST" action="{{ route('admin.student-leave.cancel', $r->id) }}" class="d-inline" onsubmit="return confirm('Cancel this approved leave?')">@csrf @method('PATCH')<button class="btn btn-sm btn-outline-warning">{{ __('Cancel') }}</button></form>
              @endif
            </td>
          </tr>
        @endforeach
