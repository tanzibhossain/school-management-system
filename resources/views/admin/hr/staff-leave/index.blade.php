...
@section('content')
  ...
  <form method="GET" class="card mb-3"><div class="card-body row g-2 align-items-end">
    ...
    <div class="col-sm-8"><button class="btn btn-sm btn-outline-primary">Filter</button>
      <a href="{{ route('admin.staff-leave.index') }}" class="btn btn-sm btn-outline-secondary">Reset</a></div>
  </div></form>

  @php $m = ['pending'=>'warning','approved'=>'success','rejected'=>'danger','cancelled'=>'secondary']; @endphp
  <div class="card"><div class="card-body">
    <table class="table table-hover align-middle w-100 js-dt">
      <thead><tr><th>Staff</th><th>Type</th><th>From</th><th>To</th><th>Days</th><th>Status</th><th class="text-end" data-orderable="false">Actions</th></tr></thead>
      <tbody>
        @foreach ($requests as $r)
          <tr>
            ...
            <td class="text-end">
              @if ($r->status === 'pending')
                <form method="POST" action="{{ route('admin.staff-leave.approve', $r->id) }}" class="d-inline">@csrf @method('PATCH')<button class="btn btn-sm btn-outline-success">Approve</button></form>
                <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#rejectModal{{ $r->id }}">Reject</button>
              @elseif ($r->status === 'approved')
                <form method="POST" action="{{ route('admin.staff-leave.cancel', $r->id) }}" class="d-inline" onsubmit="return confirm('Cancel this approved leave?')">@csrf @method('PATCH')<button class="btn btn-sm btn-outline-warning">Cancel</button></form>
              @endif
            </td>
          </tr>
        @endforeach
