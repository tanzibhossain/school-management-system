...
@section('content')
  ...
  <div class="card"><div class="card-body">
    <table class="table table-hover align-middle w-100 js-dt">
      <thead><tr><th>#</th><th>Scope</th><th>Message</th><th>Recipients</th><th>Status</th><th>When</th><th class="text-end" data-orderable="false"></th></tr></thead>
      <tbody>
        @foreach ($batches as $b)
          ...
          <tr>
            ...
            <td class="text-end"><a class="btn btn-sm btn-outline-primary" href="{{ route('admin.sms.show', $b->id) }}">Open</a></td>
          </tr>
        @endforeach
      </tbody>
    </table>
  ...
  <div class="modal fade" id="composeModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form method="POST" action="{{ route('admin.sms.store') }}">
      @csrf
      <div class="modal-header"><h5 class="modal-title">Compose SMS</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
      <div class="modal-body row g-3">
        <div class="col-md-4"><label class="form-label">Send to <span class="text-danger">*</span></label>
          <select name="scope" id="smsScope" class="form-select" required>
            <option value="all">All active students</option>
            <option value="class">A class</option>
          </select></div>
        <div class="col-md-4 sms-class d-none"><label class="form-label">Class</label>
          <select name="class_id" id="smsClass" class="form-select">
            <option value="">— select —</option>
            @foreach ($classes as $c)<option value="{{ $c->id }}">{{ $c->name }}</option>@endforeach
          </select></div>
        <div class="col-md-4 sms-class d-none"><label class="form-label">Section <span class="text-muted small">(optional)</span></label>
          <select name="section_id" id="smsSection" class="form-select" data-sel=""><option value="">All sections</option></select></div>
        <div class="col-12"><label class="form-label">Message <span class="text-danger">*</span></label>
          <textarea name="body" id="smsBody" rows="4" class="form-control" maxlength="1000" required></textarea>
          <div class="form-text"><span id="smsChars">0</span> characters</div></div>
      </div>
