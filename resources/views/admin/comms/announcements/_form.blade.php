...
<div class="modal fade" id="{{ $modalId }}" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content">
  <form method="POST" action="{{ $action }}">
    ...
    <div class="modal-body row g-3">
      <div class="col-12"><label class="form-label">Title <span class="text-danger">*</span></label>
      ...
      <div class="col-md-6"><label class="form-label">Expire at <span class="text-muted small">(optional)</span></label>
        <input type="datetime-local" name="expire_at" class="form-control" value="{{ $isEdit && $a->expire_at ? $a->expire_at->format('Y-m-d\TH:i') : '' }}"></div>
      @unless ($isEdit)
        <div class="col-md-6"><label class="form-label">Schedule for <span class="text-muted small">(optional)</span></label>
          <input type="datetime-local" name="publish_at" class="form-control"></div>
      @endunless
      <div class="col-12 d-flex gap-4">
        <div class="form-check"><input type="hidden" name="is_pinned" value="0"><input class="form-check-input" type="checkbox" name="is_pinned" value="1" id="pin{{ $modalId }}" @checked($isEdit ? $a->is_pinned : false)><label class="form-check-label" for="pin{{ $modalId }}">Pin to top</label></div>
        @unless ($isEdit)
          <div class="form-check"><input class="form-check-input" type="checkbox" name="publish_now" value="1" id="pubnow" checked><label class="form-check-label" for="pubnow">Publish immediately</label></div>
        @endunless
      </div>
    </div>
