...
<div class="modal fade" id="{{ $modalId }}" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content">
  <form method="POST" action="{{ $action }}">
    ...
    <div class="modal-body row g-3">
      ...
      <div class="col-md-3"><label class="form-label">{{ __('Type') }} <span class="text-danger">*</span></label>
        <select name="type" class="form-select" required>
          <option value="student" @selected(($isEdit ? $t->type : old('type'))==='student')>{{ __('Student') }}</option>
          <option value="staff" @selected(($isEdit ? $t->type : old('type'))==='staff')>{{ __('Staff') }}</option>
        </select></div>
      <div class="col-md-3"><label class="form-label">{{ __('Font') }}</label>
      ...
      <div class="col-md-6"><label class="form-label">{{ __('Layout') }} <span class="text-danger">*</span></label>
      ...
      <div class="col-md-3"><label class="form-label">{{ __('Background') }}</label>
