...
@section('content')
  ...
  <div class="mb-3"><a href="{{ route('admin.id-cards.index') }}" class="text-decoration-none small"><i class="bi bi-arrow-left"></i> Back to batches</a></div>
  ...
  <div class="card mb-4"><div class="card-body">
    <dl class="row mb-0">
      ...
      <dt class="col-sm-3 text-muted">Type</dt><dd class="col-sm-9 text-capitalize">{{ $batch->type }}</dd>
      <dt class="col-sm-3 text-muted">Template</dt><dd class="col-sm-9">{{ $batch->template?->name ?? '—' }}</dd>
      <dt class="col-sm-3 text-muted">Cards</dt><dd class="col-sm-9">{{ $batch->total_count }}</dd>
  ...
  <div class="card"><div class="card-header">Generated files ({{ $batch->files->count() }})</div><div class="card-body">
    ...
    @else
      <table class="table align-middle mb-0">
        <thead><tr><th>File</th><th>Cards</th><th class="text-end" data-orderable="false"></th></tr></thead>
        <tbody>
          @foreach ($batch->files as $f)
            <tr>
              <td>Sheet {{ $f->file_index + 1 }}</td>
              <td>{{ $f->card_count }}</td>
              <td class="text-end"><a class="btn btn-sm btn-outline-danger" href="{{ route('admin.id-cards.download', [$batch->id, $f->id]) }}" target="_blank"><i class="bi bi-file-pdf"></i> PDF</a></td>
            </tr>
          @endforeach
        </tbody>
      </table>
