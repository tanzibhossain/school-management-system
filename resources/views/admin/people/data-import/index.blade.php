...
@section('content')
  ...
  <div class="card"><div class="card-body">
    <table class="table table-hover align-middle w-100 js-dt">
      <thead><tr><th>#</th><th>Type</th><th>File</th><th>Rows</th><th>Imported</th><th>Skipped</th><th>Status</th><th class="text-end" data-orderable="false"></th></tr></thead>
      <tbody>
        @foreach ($batches as $b)
          <tr>
            ...
            <td class="text-end"><a class="btn btn-sm btn-outline-primary" href="{{ route('admin.data-import.show', $b->id) }}">Open</a></td>
          </tr>
        @endforeach
      </tbody>
    </table>
  ...
  <div class="modal fade" id="uploadModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form method="POST" action="{{ route('admin.data-import.store') }}" enctype="multipart/form-data">
      @csrf
      <div class="modal-header"><h5 class="modal-title">Import file</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
      <div class="modal-body row g-3">
        <div class="col-md-5"><label class="form-label">Type <span class="text-danger">*</span></label>
          <select name="type" class="form-select" required>
            <option value="student">Students</option><option value="staff">Staff</option>
          </select></div>
        <div class="col-md-7"><label class="form-label">File <span class="text-danger">*</span></label>
      ...
      <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><button class="btn btn-primary">Import</button></div>
    </form>
  </div></div></div>
@endsection
